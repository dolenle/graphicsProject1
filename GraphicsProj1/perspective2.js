...
function testCube() {
	var index=0;
	var c = [Colors.BLACK, Colors.BLUE, Colors.YELLOW, Colors.ORANGE, Colors.WHITE, Colors.GREEN];
	for(var x=-1; x<=1; x++) {
		for(var y=-1; y<=1; y++) {
			for(var z=-1; z<=1; z++) {
				rubiks.push(new Cube([x*0.41, y*0.41, z*0.41, 1.0], c));
				gl.bindBuffer( gl.ARRAY_BUFFER, cBuffer);
				gl.bufferSubData(gl.ARRAY_BUFFER, 0, flatten(cubeColors[0]));
				gl.bufferSubData(gl.ARRAY_BUFFER, 16, flatten(cubeColors[1]));
				gl.bufferSubData(gl.ARRAY_BUFFER, 32, flatten(cubeColors[2]));
				gl.bufferSubData(gl.ARRAY_BUFFER, 64, flatten(cubeColors[3]));
				gl.bufferSubData(gl.ARRAY_BUFFER, 80, flatten(cubeColors[4]));
				gl.bufferSubData(gl.ARRAY_BUFFER, 96, flatten(cubeColors[5]));
				for(var k in c) {
					for(var i=0; i<6; i++) {
